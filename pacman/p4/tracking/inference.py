...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState, numParticles=300):
	...
	randPart = random.choice(self.legalPositions)
	self.particles[randPart] += 1
  ...
  def observe(self, observation, gameState):
    ...
    for each in self.particles:
        trueDistance = util.manhattanDistance(each, pacmanPosition)
        weights[each] = emissionModel[trueDistance]*self.particles[each]
  ...
  def elapseTime(self, gameState):
    ...
    newPosDist = {}
    for each in self.particles:
        newPosDist[each] = self.getPositionDistribution(self.setGhostPosition(gameState,each))

    newParticles = util.Counter()
    for each in self.particles:
      for i in range(self.particles[each]):
        newParticles[util.sample(newPosDist[each])] += 1
    self.particles = newParticles
  ...
  def getBeliefDistribution(self):
    ...
    for each in self.particles: 
	beliefs[each] += float(self.particles[each])/self.numParticles
    return beliefs
...
class JointParticleFilter:
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
      ...
      "*** YOUR CODE HERE ***"
      for each in self.ghostAgents:
        newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), each.index, each)
        newParticle[each.index-1] = util.sample(newPosDist)
  ...
  def observeState(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    weights =util.Counter()
    #init weights as counters
    for i in range(self.numGhosts):
	weights[i]=util.Counter()
    for eachPart in self.particles:
      for eachGhost in range(self.numGhosts):
        dist = util.manhattanDistance(eachPart[eachGhost], pacmanPosition)
        weights[eachGhost][eachPart[eachGhost]] += emissionModels[eachGhost][dist]
    ...
    for i in range(self.numParticles):
      ...
      for each in range(self.numGhosts):
        if noisyDistances[each] != 999:
          newParticle.append(util.sample(weights[each]))
        else: newParticle.append(tuple([2*each+1,1]))
      #newParticle = [util.sample(weights[ghost]) for ghost in liveGhosts]
      newParticles.append(tuple(newParticle))
    self.particles = newParticles  
