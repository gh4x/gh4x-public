...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    tops = []
    for eachGhost in range(len(livingGhosts)-1):
    	#print "Doing Ghost #", eachGhost
        if livingGhosts[eachGhost+1] == True:
            ghostVals = self.ghostBeliefs[eachGhost].values() 
            #print "GV:",ghostVals
            for eachBelief in self.ghostBeliefs[eachGhost]:
            	#print "EV:",eachBelief,"=",self.ghostBeliefs[eachGhost][eachBelief],"=?",max(ghostVals),"p",(self.ghostBeliefs[eachGhost][eachBelief]==max(ghostVals))
            	if self.ghostBeliefs[eachGhost][eachBelief] == max(ghostVals):
		    		tops.append(eachBelief)
		    
    			#This is random but i think should be systematic
    			#chosenTop = tops[0] 
				#print tops
				chosenTop = random.choice(tops)
				dist = self.distancer.getDistance(chosenTop,pacmanPosition)
				if closestGhostDistance == None:
					closestGhostDistance = dist
					closestGhostLocation = chosenTop
				elif dist < closestGhostDistance:
					closestGhostDistance = dist
					closestGhostLocation = chosenTop
    ...
    for each in legal:
        if bestAction == None:
            bestAction = each
        elif closestGhostDistance > self.distancer.getDistance(closestGhostLocation, Actions.getSuccessor(pacmanPosition, each)): 
            bestAction = each   
	    "if closest ghost distance > distance to closest ghost after action take action"
    return bestAction
