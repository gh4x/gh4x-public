...
class CornersProblem(search.SearchProblem):
  ...
  def getStartState(self):
    ...
    "*** YOUR CODE HERE ***"
    myState = (self.startingPosition, [True, True, True, True])
    return myState
  ...
  def isGoalState(self, state):
    ...
    loc = state[0]
    flags = state[1]
    if loc == self.corners[0]:
      if flags[0]:
        return True
    elif loc == self.corners[1]:
      if flags[1]:
        return True
    elif loc == self.corners[2]:
      if flags[2]:
        return True
    elif loc == self.corners[3]:
      if flags[3]:
        return True
    return False

  def satisfyGoal(self, state):
    "Returns whether this search state is a goal state of the problem"
    "*** YOUR CODE HERE ***"
    loc = state[0]
    flags = state[1]
  ...
  def getSuccessors(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not(hitsWall):
        successors.extend([(((nextx, nexty),flags),action,1)])
...
def cornersHeuristic(state, problem):
  ...
  for i in range(4):
	  if flags[i]:
	      cx, cy = corners[i]
	      nums.append(abs(lx - cx) + abs(ly - cy))
	  else:
	      nums.append(999999)
  return min(nums)
  ...
  def getSuccessors(self, state):
    ...
    for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not self.walls[nextx][nexty]:
        ...
        nextFood[x][y] = False ############looking at food should not eat food (neccesary fix)
